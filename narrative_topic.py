...
from event_narratives import fetch_narratives
from navsafecen import SafecenStopWords
...
dataset = fetch_narratives(shuffle=True, random_state=1,
                             remove=('headers', 'footers', 'quotes'),
                             categories=['ship'])

n_samples = len(dataset.data)   # Use all data files

vectorizer = TfidfVectorizer(max_df=0.95, min_df=2, max_features=n_features,
#                            stop_words=('english'))
                             stop_words=SafecenStopWords().english)
